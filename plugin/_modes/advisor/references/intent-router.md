...
**Master.yaml prelude (runs before all other /init steps):** Read `_orgs/<org-slug>/master.yaml` via `master-yaml-ops.md` → `read_master(org_slug)`. If no master exists for this slug, the org is not provisioned in the backend — provisioning is admin-path only (P4b D4). Surface "org `<slug>` is not set up in the backend" and stop; escalate to admin provisioning. Do NOT create a local `master.yaml` or `_orgs/index.yaml` — that would create an org the backend never learns of (divergence), per `master-yaml-ops.md` and `init-mode-protocol.md`. Validate post-read per `master-yaml-ops.md` → `validate_against_schema`. Surface tree view of inheritable groups with freshness signals. Check pull-notifications (filter `decision_log` for cross-skill events since last compensation-advisor init). Then proceed to the 10-step /init flow in `init-mode-protocol.md`.
...
Reads `engagement_get_master {org_slug}`; verifies the cycle exists in `master.cycles[]` and status ≠ `closed`. Surfaces a confirmation tree showing all skills that touched the cycle and whether each has open work. On confirm: append this skill's self-rollup via `engagement_append_decision {decision_type: cycle_closed, cycle_slug, tags: [cycle-close, advisor-rollup], summary}` (summary includes scenario shipped, council runs logged, glossary terms promoted, costing artifacts produced); then close the cycle by resending the FULL record read from `master.cycles[]`: `engagement_put_cycle {org_slug, cycle_slug, status: closed, opened_date, opened_by_skill, primary, cycle_dir, closed_date: <today>}`. `engagement_put_cycle` is a blind UPSERT with no version guard — a partial call clobbers the cycle's stored `opened_date`/`opened_by_skill`/`primary`/`cycle_dir`, so all server-required fields are carried through from the read (as `close-flow.md` already does). Does NOT write roll-ups for sibling skills — those are lazy-written on their own next `/init`.
...
Reads `engagement_get_master {org_slug}`; locates the cycle in `master.cycles[]`. Flips the cycle from `closed` back to `drafting` by resending the FULL record read from `master.cycles[]`: `engagement_put_cycle {org_slug, cycle_slug, status: drafting, opened_date, opened_by_skill, primary, cycle_dir}`. `engagement_put_cycle` is a blind UPSERT with no version guard — resend all server-required fields from the read (as `close-flow.md` already does), or a partial call clobbers the cycle's stored `opened_date`/`opened_by_skill`/`primary`/`cycle_dir`. Then `engagement_append_decision {decision_type: cycle_reopened, cycle_slug, refs: {related_decision_ids: [<original cycle_closed id>]}}`. The cycle's local `cycle_dir` is amended in place (non-schema artifacts, no cycle versioning).
...
Reads `engagement_get_master {org_slug}`; locates the cycle in `master.cycles[]`. Makes the named cycle primary by resending the FULL record read from `master.cycles[]` with `primary` set to `true`: `engagement_put_cycle {org_slug, cycle_slug, status, opened_date, opened_by_skill, primary: true, cycle_dir}` (carry `status`/`opened_date`/`opened_by_skill`/`cycle_dir` through from the read — `engagement_put_cycle` is a blind UPSERT with no version guard, so a partial call clobbers the cycle's other stored fields, as `close-flow.md` already guards against). The server **atomically demotes the previously-primary cycle** to `primary: false` (exactly one cycle holds `primary: true` at any time — enforced server-side, not by a second client write). Then `engagement_append_decision {decision_type: cycle_primary_switched, cycle_slug}`.
