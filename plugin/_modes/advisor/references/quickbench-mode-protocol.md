...
1. **`mcp__market__search_roles(query_text, province)`** — discover the right role first. Read the top match's **`coverage_status`** (`full` | `thin_data` | `no_data` | `nearest_noc`), not `match_score` alone — the server superseded the old `score >= 0.7` proxy with `coverage_status` (and, once the deployed facade carries it, an `abstain` verdict). Capture the `role_id` only when `weak_match: false` AND `coverage_status` is `full` or `thin_data`. When `coverage_status` is `nearest_noc` (a neighbouring occupation, not the one asked for) or `no_data`, do NOT silently use it — surface the coverage to the user and confirm. Carry the coverage label into the mini-report so a `nearest_noc` / `thin_data` match is never presented as a `full`-coverage one. If `weak_match: true`, do NOT pick a `role_id` — pass the original title to the next call.
2. **`mcp__market__get_role_intelligence(role_id or role, province, percentiles=[10,25,50,75,90])`** — primary call. Returns:
   - `benchmarks.statcan.{p10, p25, p50, p75, p90}` — government wage-survey percentiles. **Each percentile is an enriched OBJECT, not a bare scalar**: `{value, unit, confidence_label, caveat}`. Read `.value` for the dollar figure; `confidence_label` (e.g. SURVEY/LOW) and `caveat` are load-bearing — surface them per Output Format.
   - `benchmarks.live_posting_start_rate.{p10, ...}` and `live_posting_top_rate.{p10, ...}` — live job posting rates (entry vs top of role's posting range), same enriched-object shape. **May be `null` when `postings_basis: suppressed`** — that means the postings are below the reporting floor, NOT $0 / no market.
   - `census_grain` (`province` | `national`) and `postings_basis` (`province` | `residual_national` | `suppressed`) — block-level geography/basis flags. When either is not `province`, the figures are a Canada-wide fallback: annotate the affected column and correct the geography in the header (Output Format § Trust flags). This is the exact failure C06 exists to prevent.
   - `market_drift_pct`, `transparency_tier`, `confidence_tier`, `recent_posting_count`, `salary_disclosure_rate`
   - `summary_with_caveats` — a top-level instruction string ("Each figure carries a co-located caveat; surface it alongside any number you cite."). Honor it verbatim in the mini-report.
...
```
**[Role] — [Geography], [Date]**
(Geography = the province the user asked for — UNLESS `census_grain: national` or `postings_basis: residual_national`, in which case write "Canada (national fallback)" and flag it below. Never show a Canada-wide figure under a province header.)
...
Render each cell from `.value` of the percentile object. **Confidence:** StatCan [`confidence_label`, e.g. SURVEY] · Live postings [`confidence_tier`, e.g. HIGH / SUPPRESSED]. Quote the shared caveat once (the server's `summary_with_caveats` requires it), e.g. "StatCan figures are survey estimates, not derived from job postings."

**Trust flags — render only the ones present in the response:**
- `postings_basis: suppressed` → the two live-posting columns come back `null`. Render them as "— (suppressed)", never $0. Add: "Live postings are below the reporting floor — suppressed, not zero market."
- `census_grain: national` OR `postings_basis: residual_national` → the figures are Canada-wide, not the requested province. The header reads "Canada (national fallback)"; add: "No province-level data for [Province] — showing national. Don't position provincial pay on this without flagging it." (C06.)
...
or below P50? Anything notable about CBA scales vs non-union? If any 
figure carried a caveat or a trust flag (suppressed, national fallback, 
low confidence), name it here in plain language — the reader must not 
position pay on a flagged number without knowing it is flagged.]
...
- **Chat delivery (always)**: deliver the file artifact in the chat for download; the user saves it to their own archive.
- **Local archive (optional)**: the quickbench mini-report is non-schema output — mirror it to `quickbench-archive/{YYYY-MM-DD}-{role-slug}-{province}.md` in the local `$STATE_ROOT` state (deliverables and scratch are local, per `references/persistence-and-ledger.md` § Where each thing lives, D3; resolved per `references/library-resolution.md`). There is no backend archive tool — the quickbench archive never touches the `market` MCP backend.
