...
Layouts 1–12 are the core deck spine; layouts 13–18 cover specialist/long-form needs (charts, methodology, multi-province, risks, expanded TOC, ≥8-role positioning); layouts 19–21 cover narrative-first decks (force/option grids, narrative dividers, exec callouts) where the deck arc is the framing rather than the data.
...
| 19 | `19_FORCE_OPTION_CARD_GRID` | Three-column narrative framing (force / option / question) | 3 equal-width cards on light tint background. Each card has small ALL-CAPS label (FORCE/OPTION/QUESTION), title, body copy, and small footer note. Used in `narrative-frame-only` mode and any pre-engagement deck where 3 parallel framings need equal visual weight. |
| 20 | `20_NARRATIVE_SECTION_DIVIDER` | Visual section break for narrative-first decks | Lighter-weight than Layout 4. Pale tint background, thin accent rule above title, small ALL-CAPS section label, large centered narrative title, optional one-line context. No section number, no decorative numeral. Use Layout 4 for data-deck dividers; this for narrative-deck dividers. |
| 21 | `21_EXECUTIVE_SUMMARY_CALLOUT` | Single high-impact summary slide for VP/exec audiences | One-sentence headline (large, bold, top), 3 supporting bullets (middle), small footer attribution (bottom). Used at the top of pre-reads and at the close of approval pitches when one slide must carry the whole argument. |

### Layout family

Group layouts by use-case to scan by intent rather than by number:

| Family | Layouts | When to reach for |
|---|---|---|
| **Data layouts** | 6, 7, 8, 9, 13, 14, 15, 16, 17 | Tables, charts, market analysis, costing, methodology, risks. The full-engagement spine. |
| **Narrative layouts** | 1, 3, 5, 10, 12, 19, 20, 21 | Cover, callouts, two-column comparisons, next steps, blank-flex, force/option grids, narrative dividers, exec callouts. The pre-engagement spine and any deck where the arc carries the argument. |
| **Structural layouts** | 2, 4, 11 | TOC, section divider (data-deck), closing decision ask. |
| **Utility layouts** | 18 | Expanded TOC for >6 sections. |
...
- **Section dividers:** Layout 4 for data-deck dividers (full-bleed green, section number, decorative numeral); Layout 20 for narrative-deck dividers (pale tint, no number, centered narrative title).
- **Charts:** Layout 13 is the home for any chart. Don't chart-jam into Layout 12 (blank/flex).
- **Three-column framings:** Layout 5 for two-side comparisons (option A vs B); Layout 19 for three-column force/option/question framings.
- **Exec summary slides:** Layout 21 for one-slide-carries-the-whole-argument moments at the top of pre-reads or close of approval pitches.
...
## `build_template.js` vs `build_deck_skeleton.js`

The brand kit ships two entry points. They share the master files but differ in what they do with them:

| Entry point | What it does | When to use |
|---|---|---|
| `build_template.js` | Registers all masters AND adds the demo slides for each (so the produced .pptx has 21 slides showcasing every layout). | Regenerating the template artifact for orientation, visual reference, or PowerPoint editing by a colleague. The output is the **template file** itself. |
| `build_deck_skeleton.js` | Registers all masters via `defineMaster()` only — **no demo slides**. Returns `{ pres, deps }` for inline use by a deck-builder script. | Building a real deck from scratch with skill-generated content. The output is **your real deck**, not a template. |

Each master file exports three functions to support this split: `defineMaster(deps)` (slide-master only), `addDemoSlide(deps)` (demo slide only), and `register(deps)` (the thin wrapper that calls both, kept for `build_template.js` backward compatibility).

## Working in scratch

When working in scratch (e.g., copying the brand kit into a sandbox to iterate without touching the canonical `template_assets/branding/_default/`), **preserve the `branding/<org-slug>/` parent directory**. The build script's resolver assumes the parent directory carries the org-slug name, NOT bare `branding/`. Copy `template_assets/branding/_default/` to scratch as `branding/_default/`, NOT as `branding/`. If you flatten the structure, `require.resolve('../${orgSlug}/theme/palette.json')` resolves to a path that doesn't exist and the build fails with no useful diagnostic.
